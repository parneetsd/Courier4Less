...
<Postalcode>'.$dest_postal.'</Postalcode>
    ...
    if ( empty($quotes) ) {
        wp_die('<h2>No DHL quotes found.</h2>');
    }

    // RANDOM ORDER NUMBER
    $order_number = 'C4L' . rand(10000,99999);

    echo '<div style="max-width:900px;margin:40px auto;font-family:Arial,sans-serif;">';
    echo '<h2>DHL Estimate Results</h2>';

    echo '<form action="https://courier4less.com/secure-payment" method="GET">';

    echo '<table style="width:100%;border-collapse:collapse;">';
    echo '<tr>
            <th style="border:1px solid #ccc;padding:10px;">Select</th>
            <th style="border:1px solid #ccc;padding:10px;">Service</th>
            <th style="border:1px solid #ccc;padding:10px;">DHL Price</th>
            <th style="border:1px solid #ccc;padding:10px;">Our Price (30% OFF)</th>
            <th style="border:1px solid #ccc;padding:10px;">Transit Days</th>
            <th style="border:1px solid #ccc;padding:10px;">Delivery Date</th>
          </tr>';
    ...
    foreach ( $quotes as $index => $quote ) {
        ...
        $price = (float) ($quote->WeightCharge ?? 0);
        ...
        echo '<td style="border:1px solid #ccc;padding:10px;text-align:center;">
                <input type="radio" name="selected_service" value="'.esc_attr($service).'" required>
              </td>';
        echo '<td style="border:1px solid #ccc;padding:10px;">'.esc_html($service).'</td>';
        echo '<td style="border:1px solid #ccc;padding:10px;">'.$currency.' '.number_format($price,2).'</td>';
        echo '<td style="border:1px solid #ccc;padding:10px;color:green;"><strong>'.$currency.' '.number_format($discount_price,2).'</strong></td>';
        echo '<td style="border:1px solid #ccc;padding:10px;">'.$transit_days.'</td>';
        echo '<td style="border:1px solid #ccc;padding:10px;">'.$delivery_date.'</td>';
        echo '</tr>';
    }
    ...
    echo '<button type="submit" style="background:#1e3a8a;color:#fff;padding:12px 20px;border:none;border-radius:5px;cursor:pointer;">Place Order</button>';
    echo '</div>';
